class Api::V1::LessonsController < Api::V1::ApplicationController
  ...
  before_action :set_chapter, only: %i(index create create_lesson_assessment update_lesson_assessment show update update_positions destroy)
  before_action :set_course, only: %i(introduction introduction_index update_introduction)

  # GET courses/:course_id/chapters/:chapter_id/lessons
  def index
    @lessons = Lesson.includes(%i(chapter additional_resource content)).where(chapter_id: @chapter.id).order_by(position: :asc)
  ...
  # PUT courses/:course_id/lessons/introduction
  ...
  def update_positions
    ...
    @lessons = Lesson.includes(%i(chapter events notifications)).where(chapter_id: @chapter.id).find(params[:positions])
    @lessons.each do |lesson|
      lesson.update_attributes(position: positions_hash[lesson.id.to_s].to_i)
    end
    @lessons = Lesson.includes(%i(chapter events notifications)).where(chapter_id: @chapter.id).order_by(position: :asc)
  ...
  private
    ...
    def lesson_assessment_params
      params.require(:assessment).permit(:title, :description, :content_id, :additional_resource_id, :prerequisite, :downloadable, :type, :can_discuss, :status, :position, questions: [:type, :text, choices: %i(value is_correct)])
    end

    def lesson_assessment_update_params
      params.require(:assessment).permit(:title, :description, :content_id, :additional_resource_id, :prerequisite, :downloadable, :type, :can_discuss, :status, questions: [:type, :text, choices: %i(value is_correct)])
    end
end
